...
describe('SendDocumentCommand', () => {
  test('should create command with all properties', () => {
    const command = new SendDocumentCommand('inst-1', 'jid1', Buffer.from('doc-data'), 'file.pdf', 'application/pdf', 'My doc')
  ...
  test('should create command without caption', () => {
    const command = new SendDocumentCommand('inst-1', 'jid1', Buffer.from('doc-data'), 'file.pdf', 'application/pdf')
...
describe('DocumentSender', () => {
  test('should send document successfully', async () => {
    ...
    const command = new SendDocumentCommand('inst-1', 'jid1', Buffer.from('doc-data'), 'file.pdf', 'application/pdf', 'Caption')

    await sender.execute(command)

    expect(mockAdapter.messaging.sendDocument).toHaveBeenCalledWith('jid1', command.document, 'file.pdf', 'application/pdf', 'Caption')
  })
  ...
  test('should throw when instance not found', async () => {
    ...
    const command = new SendDocumentCommand('inst-1', 'jid1', Buffer.from('doc-data'), 'f.pdf', 'application/pdf')
