...
describe('SendAudioCommand', () => {
  test('should create command with all properties', () => {
    const command = new SendAudioCommand('inst-1', 'jid1', Buffer.from('audio-data'), true, 'audio/ogg')
  ...
  test('should create command with defaults', () => {
    const command = new SendAudioCommand('inst-1', 'jid1', { url: 'https://example.com/audio.ogg' })
...
describe('AudioSender', () => {
  test('should send audio successfully', async () => {
    ...
    expect(mockAdapter.messaging.sendAudio).toHaveBeenCalledWith('jid1', command.audio, true, 'audio/ogg')
  })
