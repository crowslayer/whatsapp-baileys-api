...
describe('DescriptionUpdater', () => {
  ...
  test('execute calls adapter.groups.updateGroupDescription', async () => {
    ...
    const mockAdapter = { groups: { updateGroupDescription: vi.fn().mockResolvedValue(undefined) } }
    ...
    expect(mockAdapter.groups.updateGroupDescription).toHaveBeenCalledWith('group-123', 'New Description')
  })
  ...
  test('throws WhatsAppConnectionError when instance not found or not connected', async () => {
    ...
    await expect(updater.execute(instanceId, 'group-123', 'Desc')).rejects.toThrow(WhatsAppConnectionError)
  })
