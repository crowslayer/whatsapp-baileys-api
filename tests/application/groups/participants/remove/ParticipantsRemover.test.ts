...
describe('ParticipantsRemover', () => {
  test('execute removes participants', async () => {
    ...
    const mockAdapter = {
      groups: { removeParticipantsFromGroup: vi.fn().mockResolvedValue(undefined) },
    };
    ...
    expect(mockAdapter.groups.removeParticipantsFromGroup).toHaveBeenCalledWith('group-123', [
      'jid1',
      'jid2',
    ]);
  });
