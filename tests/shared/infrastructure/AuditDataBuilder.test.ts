...
describe('AuditDataBuilder', () => {
  ...
  test('should add user info', () => {
    const audit = new AuditDataBuilder('READ', 'INSTANCE').withUser('user-1', 'John').build();
    expect(audit.userId).toBe('user-1');
    expect(audit.userName).toBe('John');
  });

  test('should add resource id', () => {
    const audit = new AuditDataBuilder('DELETE', 'FLOW').withResourceId('flow-123').build();
  ...
  test('should add details', () => {
    const audit = new AuditDataBuilder('CREATE', 'FLOW').withDetails({ name: 'Test Flow' }).build();
