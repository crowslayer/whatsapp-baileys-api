...
function makeRuntimeManager({
  ...
  logger = { info: vi.fn(), warn: vi.fn(), error: vi.fn(), fatal: vi.fn(), debug: vi.fn(), trace: vi.fn() },
} = {}) {
...
describe('RuntimeManager', () => {
  test('should start instance', async () => {
    ...
    const registry = { register: vi.fn(), get: vi.fn().mockImplementation(() => { throw new Error('not found') }), remove: vi.fn() }
  ...
  test('should not start twice', async () => {
    const mockRuntime = { start: vi.fn(), onDisconnect: vi.fn() }
    const registry = { register: vi.fn(), get: vi.fn().mockReturnValue(mockRuntime), remove: vi.fn() }
