...
describe('InstancesCreator', () => {
  test('should throw when name already exists', async () => {
    const mockRepo = { findByName: vi.fn().mockResolvedValue({}), save: vi.fn(), findById: vi.fn(), update: vi.fn(), findAll: vi.fn() }
  ...
  test('should create instance successfully', async () => {
    const mockRepo = { findByName: vi.fn().mockResolvedValue(null), save: vi.fn(), findById: vi.fn(), update: vi.fn(), findAll: vi.fn() }
    const mockRuntime = { start: vi.fn() }
    const creator = new InstancesCreator(mockRepo, mockRuntime)
