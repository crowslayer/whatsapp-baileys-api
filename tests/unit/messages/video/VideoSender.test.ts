...
describe('SendVideoCommand', () => {
  test('should create command with all properties', () => {
    const command = new SendVideoCommand('inst-1', 'jid1', Buffer.from('video-data'), 'My caption', true, 'video.mp4')
...
describe('VideoSender', () => {
  test('should send video successfully', async () => {
    ...
    const command = new SendVideoCommand('inst-1', 'jid1', Buffer.from('video-data'), 'Caption', true)

    await sender.execute(command)

    expect(mockAdapter.messaging.sendVideo).toHaveBeenCalledWith('jid1', command.video, 'Caption', true)
  })
