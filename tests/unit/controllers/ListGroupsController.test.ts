...
import { ListGroupsController } from '../../../src/infrastructure/http/controllers/groups/ListGroupsController';
...
describe('ListGroupsController', () => {
  ...
  beforeEach(() => {
    ...
    controller = new ListGroupsController(mockQueryBus);
    req = {
  ...
  test('should ask query bus and return 200', async () => {
    mockQueryBus.ask.mockResolvedValue(GroupsResponse.create([{ id: 'group-1', name: 'Test' }]));
  ...
  test('should handle empty groups list', async () => {
    mockQueryBus.ask.mockResolvedValue(GroupsResponse.create([]));
