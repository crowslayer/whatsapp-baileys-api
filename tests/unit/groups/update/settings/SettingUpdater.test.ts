...
describe('SettingUpdater', () => {
  ...
  test('execute with announcement calls adapter.groups.updateGroupSettings', async () => {
    ...
    expect(mockAdapter.groups.updateGroupSettings).toHaveBeenCalledWith('group-123', 'announcement')
  })
  ...
  test('execute with not_announcement calls adapter', async () => {
    ...
    expect(mockAdapter.groups.updateGroupSettings).toHaveBeenCalledWith('group-123', 'not_announcement')
  })
  ...
  test('throws WhatsAppConnectionError when instance not found or not connected', async () => {
    ...
    await expect(updater.execute(instanceId, 'group-123', 'announcement')).rejects.toThrow(WhatsAppConnectionError)
  })
