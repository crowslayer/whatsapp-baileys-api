...
describe('ParticipantsDemoter', () => {
  ...
  test('execute demotes participants successfully', async () => {
    ...
    expect(mockAdapter.groups.demoteParticipants).toHaveBeenCalledWith('group-123', ['jid1', 'jid2'])
  })
  ...
  test('throws WhatsAppConnectionError when instance not found', async () => {
    ...
    await expect(demoter.execute(instanceId, 'group-123', ['jid1'])).rejects.toThrow(WhatsAppConnectionError)
  })
