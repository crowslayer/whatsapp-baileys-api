...
describe('ParticipantsAggregator', () => {
  ...
  test('should add participants successfully', async () => {
    ...
    const mockAdapter = { groups: { addParticipantsToGroup: vi.fn().mockResolvedValue(undefined) } }
    ...
    expect(mockAdapter.groups.addParticipantsToGroup).toHaveBeenCalledWith('group-id', ['jid1', 'jid2'])
  })
})
